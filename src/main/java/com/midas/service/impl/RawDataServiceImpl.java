 ...
 *
 */
@Service("rDataService")
public class RawDataServiceImpl extends DataBase implements DataService {

    private Logger logger = LoggerFactory.getLogger(RawDataServiceImpl.class);

    @Autowired
    @Qualifier("rawDataDao")
    ...
    public int insert(Map<String, Object> map, String valLabel) {
        ...
        standingMap.put("data_type", DataType.RAW_DATA.getType());
        standingMap.put("work_area", map.get("work_area"));
        standingMap.put("construction_unit", map.get("construction_unit"));
        standingMap.put("construction_year", map.get("construction_year"));
    ...
    public List<String> getFilepath(String volLabel,String dataSource) {
        ...
        for (Map<String, Object> map : list) {
            // resultList.add(ObjectUtils.toString(map.get("work_area"))+"/"+ObjectUtils.toString(map.get("tape_number")));
            List<String> fileAll = getAllFiles(ObjectUtils.toString(map.get("work_area")),
                    ObjectUtils.toString(map.get("tape_number")), StringTools.paserInt(map.get("tape_size")));
