...
<script type="text/javascript">
	$(document).ready(function() {
		...
		$(".excel").click(function(){
			window.location.href = '<%=basePath%>/tData/excel.do?project_name='+$("#project_name").val()+'&filing_unit='+$("#filing_unit").val()+'&filing_date='+$("#filing_date").val();
		});
...
<body>
	...
	<div class="place">
		...
		<ul class="placeul">
			...
			<li><a href="<%=basePath%>/tData/list.do">二维数据列表</a></li>
		</ul>
	</div>
	...
	<div class="formbody">
		<div id="usual1" class="usual">
			...
			<form action="<%=basePath%>/tData/list.do" method="post">
			<ul class="seachform">
				<li>
					<label>&nbsp;&nbsp;&nbsp;&nbsp;项目名称</label>
			...
			<div class="pagin">
		        ...
		        <ul class="paginList">
			        ...
			        <c:if test="${pageInfo.pageNum != 1 }">
			        	<li class="paginItem"><a href="<%=basePath%>/tData/list.do?pageNum=${pageInfo.pageNum - 1}&project_name=${project_name }&filing_unit=${filing_unit }&filing_date=${filing_date }"><span class="pagepre"></span></a></li>
			        </c:if>
			        <c:forEach items="${pageInfo.navigatepageNums }" var="num" varStatus="status">
			        	<c:if test="${status.count < 5 }">
			        		<li class="paginItem"><a charset="utf-8" href="<%=basePath%>/tData/list.do?pageNum=${num}&project_name=${project_name }&filing_unit=${filing_unit }&filing_date=${filing_date }">${num }</a></li>
			        	</c:if>
			        </c:forEach>
			        ...
			        <c:if test="${pageInfo.pages > pageInfo.pageNum}">
			       	 	<li class="paginItem"><a href="<%=basePath%>/tData/list.do?pageNum=${pageInfo.pageNum + 1}&project_name=${project_name }&filing_unit=${filing_unit }&filing_date=${filing_date }"><span class="pagenxt"></span></a></li>
			        </c:if>
		        </ul>
		    </div>
		</div>
	...
	<div class="tipnew">
		...
		<form action="<%=basePath%>/data/upload.do" method="post" enctype="multipart/form-data">
			<input name="dataType" value="T" type="hidden" />
			<div class="tipinfo">
