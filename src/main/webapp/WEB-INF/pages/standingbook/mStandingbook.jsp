...
<script type="text/javascript">
	$(document).ready(function() {
		...
		$(".excel").click(function(){
			window.location.href = '<%=basePath%>/standingbook/mExcel.do?work_area='+$("#work_area").val()+'&construction_year='+$("#construction_year").val()+'&startDate='+$("#startDate").val()+'&endDate='+$("#endDate").val()+'&type='+$("#type").val();
		});
...
<body>
	...
	<div class="place">
		...
		<ul class="placeul">
			...
			<li><a href="<%=basePath%>/standingbook/mData.do">原始数据台账信息</a></li>
		</ul>
	</div>
	...
	<div class="formbody">
		<div id="usual1" class="usual">
			...
			<form action="<%=basePath%>/standingbook/dData.do" method="post">
			<ul class="seachform">
				<li>
					<label>&nbsp;&nbsp;&nbsp;&nbsp;项目名</label>
				...
				<tbody>
					<c:forEach items="${pageInfo.list }" var="page" varStatus="status">
						<tr class="${status.count % 2 == 0 ? '' : 'brown'}">
							...
							<td>
							 <shiro:hasPermission name="burn:mData">
							<a href="<%=basePath%>/standingbook/delete.do?sid=${page.sid}&dataType=R&url=standingbook/mData.do">删除</a>
			...
			<div class="pagin">
		        ...
		        <ul class="paginList">
			        ...
			        <c:if test="${pageInfo.pageNum != 1 }">
			        	<li class="paginItem"><a href="<%=basePath%>/standingbook/mData.do?pageNum=${pageInfo.pageNum - 1}&project_name=${project_name }&filing_unit=${filing_unit }&filing_date=${filing_date }"><span class="pagepre"></span></a></li>
			        </c:if>
			        <c:forEach items="${pageInfo.navigatepageNums }" var="num" varStatus="status">
			        	<c:if test="${status.count < 5 }">
			        		<li class="paginItem"><a charset="utf-8" href="<%=basePath%>/standingbook/mData.do?pageNum=${num}&project_name=${project_name }&filing_unit=${filing_unit }&filing_date=${filing_date }">${num }</a></li>
			        	</c:if>
			        </c:forEach>
			        ...
			        <c:if test="${pageInfo.pages > pageInfo.pageNum}">
			       	 	<li class="paginItem"><a href="<%=basePath%>/standingbook/mData.do?pageNum=${pageInfo.pageNum + 1}&project_name=${project_name }&filing_unit=${filing_unit }&filing_date=${filing_date }"><span class="pagenxt"></span></a></li>
			        </c:if>
		        </ul>
		    </div>
		</div>
