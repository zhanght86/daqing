...
<script type="text/javascript">
	$(document).ready(function() {
		...
		$(".excel").click(function(){
			window.location.href = '<%=basePath%>/standingbook/rawExcel.do?work_area='+$("#work_area").val()+'&construction_year='+$("#construction_year").val()+'&startDate='+$("#startDate").val()+'&endDate='+$("#endDate").val()+'&type='+$("#type").val();
		});
...
<body>
	...
	<div class="place">
		...
		<ul class="placeul">
			...
			<li><a href="<%=basePath%>/standingbook/rawData.do">原始数据台账信息</a></li>
		</ul>
	</div>
		...
		<div id="usual1" class="usual">
				...
				<tbody>
					<c:forEach items="${pageInfo.list }" var="page" varStatus="status">
						...
						<%-- 	<td>${page.construction_unit}</td>
							...
							<td>
							 <shiro:hasPermission name="burn:raw">
								<a  onclick="return confirm('确定要删除么？');" href="<%=basePath%>/standingbook/delete.do?sid=${page.sid}&dataType=R&url=standingbook/rawData.do">删除</a>
			...
			<div class="pagin">
		        ...
		        <ul class="paginList">
			        ...
			        <c:if test="${pageInfo.pageNum != 1 }">
			        	<li class="paginItem"><a href="<%=basePath%>/standingbook/rawData.do?pageNum=${pageInfo.pageNum - 1}&project_name=${project_name }&filing_unit=${filing_unit }&filing_date=${filing_date }"><span class="pagepre"></span></a></li>
			        </c:if>
			        <c:forEach items="${pageInfo.navigatepageNums }" var="num" varStatus="status">
			        	<c:if test="${status.count < 5 }">
			        		<li class="paginItem"><a charset="utf-8" href="<%=basePath%>/standingbook/rawData.do?pageNum=${num}&project_name=${project_name }&filing_unit=${filing_unit }&filing_date=${filing_date }">${num }</a></li>
			        	</c:if>
			        </c:forEach>
			        ...
			        <c:if test="${pageInfo.pages > pageInfo.pageNum}">
			       	 	<li class="paginItem"><a href="<%=basePath%>/standingbook/rawData.do?pageNum=${pageInfo.pageNum + 1}&project_name=${project_name }&filing_unit=${filing_unit }&filing_date=${filing_date }"><span class="pagenxt"></span></a></li>
			        </c:if>
		        </ul>
		    </div>
		</div>
